...
class ProductService {
  final CollectionReference<Map<String, dynamic>> _productsCollectionRef = FirebaseFirestore.instance.collection('products');
  ...
  List<Product> filterByDescription(List<Product> products, String query) {
    return products.where((element) => element.description!.toLowerCase().contains(query.toLowerCase())).toList();
  }
  ...
  Stream<QuerySnapshot>? getAllProductsStream(String? category, String? typesChocolate, double? minPrice, double? maxPrice) {
  ...
  Stream<QuerySnapshot>? getAllProductsStreamWhitoutRestrictions(String? category, String? typesChocolate) {
    try {
      if(category == null && typesChocolate == null) {
        return _productsCollectionRef
            .orderBy('description', descending: false)
  ...
  Future<bool> addProduct({
    ...
    try {
      ...
      if (imageFile != null) {
        ...
        await _uploadImage(lastUserToUpdate.get('username'), productDocumment.id, imageFile, platIsWeb);
      }
  ...
  Future<bool> updateProduct({
    ...
    try {
      ...
      if (imageFile != null) {
        final lastUserToUpdate = await product.lastUserToUpdate!.get();
        await _uploadImage(lastUserToUpdate.get('username'), productId, imageFile, platIsWeb);
      }
      return Future.value(true);
    } catch (e) {
      debugPrint(e.toString());
  ...
  _uploadImage(String? uploadedBy, String? productId, dynamic imageFile, bool platIsWeb) async {
    ...
    try {
      Reference storageRef = _firebaseStorage.ref().child('products').child(productId ?? 'product').child(uuid);
