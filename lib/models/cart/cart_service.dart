...
class CartService extends ChangeNotifier {
  ...
  final CollectionReference<Map<String, dynamic>> _cartCollectionRef = FirebaseFirestore.instance.collection('cart');
  ...
  Future<List<CartItem>> getAllCartItems(DocumentReference userRef) async {
    try {
     List<CartItem> cartItemList = [];
      final QuerySnapshot<Map<String, dynamic>> querySnapshot = await _cartCollectionRef
          .doc(userRef.id)
  ...
  Future<bool> addCartItem(DocumentReference userRef, CartItem cartItem) async {
    try {
      ...
      try {
        doc = await _cartCollectionRef.doc(userRef.id).collection(_cartItemcollectionName).get()
        .then((value) => value.docs.firstWhere((element) => element.get('productRef') == cartItem.productRef));
      } catch (e) {
        debugPrint(e.toString());
      }
  ...
  Future<bool> updateCartItem(DocumentReference userRef, CartItem cartItem) async {
    try {
      await _cartCollectionRef
          .doc(userRef.id)
          .collection(_cartItemcollectionName)
