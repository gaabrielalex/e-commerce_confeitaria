...
class _AddressEditPageState extends State<AddressEditPage> {
      ...
      Future.delayed(const Duration(milliseconds: 2500), () {
        if (addressToUpdate == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            CSSnackBar(
              text: 'Erro ao carregar endereço. Verifique sua conexão com a internet ou se acessou a página corretamente.',
  ...
  Widget build(BuildContext context) {
    return DefaultFormOfAddress(
      ...
      isStream: true,
      stream: _addressService.getAddressStreamById(widget.addressId!),
      ...
      onSave: () async {
        ...
        bool response = await _addressService.updateAddress(addressToUpdate!.id!,
            Address(
        ...
        if (response) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            CSSnackBar(
              text: 'Houve um erro ao atualizar o endereço. Caso o erro persista, entre em contato com o suporte.',
              actionType: CSSnackBarActionType.error,
            ),
          );
        }
      ...
      beforeBuildingContent: (snapshot) {
        try {
          addressToUpdate = Address.fromJson(snapshot.data!);
        } catch (e) {
